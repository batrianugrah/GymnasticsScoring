document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateTable(scores) {
        ...
        // Tambahkan baris baru untuk setiap skor
        scores.forEach(score => {
            const row = document.createElement('tr');
            row.innerHTML = `
                <td><strong>${score.rank}</strong></td>
                <td>${score.nama_peserta}</td>
                <td>${score.nama_daerah}</td>
                <td>${score.alat}</td>
                <td>${score.nilai_d}</td>
                <td>${score.nilai_e}</td>
                <td>${score.nilai_a}</td>
    ...
    // Panggil fetchScores setiap 3 detik
    fetchScores(); // Panggil sekali saat halaman dimuat
